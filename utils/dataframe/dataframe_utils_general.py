...
class DataframeUtilsGeneral:
    ...
    def __apply_sampler(
        ...
        return pd.concat(
            [
                pd.DataFrame(resampled_features),
                pd.DataFrame(resampled_target, columns=[target_col]),
            ],
            axis=1,
        )
    ...
    def smote(df: pd.DataFrame, target_col: str, k_neighbors: int = 5) -> pd.DataFrame:
        """Applies SMOTE."""
        return DataframeUtilsGeneral.__apply_sampler(
            df, target_col, SMOTE, k_neighbors=k_neighbors
        )

    @staticmethod
    def borderline_smote(
        df: pd.DataFrame, target_col: str, k_neighbors: int = 5
    ) -> pd.DataFrame:
        """Applies BorderlineSMOTE."""
        return DataframeUtilsGeneral.__apply_sampler(
            df, target_col, BorderlineSMOTE, k_neighbors=k_neighbors
        )
    ...
    def svm_smote(
        ...
        """Applies SVM-SMOTE."""
        return DataframeUtilsGeneral.__apply_sampler(
            df, target_col, SVMSMOTE, k_neighbors=k_neighbors
        )
    ...
    def adasyn(df: pd.DataFrame, target_col: str, n_neighbors: int = 5) -> pd.DataFrame:
        """Applies ADASYN."""
        return DataframeUtilsGeneral.__apply_sampler(
            df, target_col, ADASYN, n_neighbors=n_neighbors
        )
    ...
    def bootstrap_resampling(
        df: pd.DataFrame, target_col: str, n_samples: int = None
    ) -> pd.DataFrame:
